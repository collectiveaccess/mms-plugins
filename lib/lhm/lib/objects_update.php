...
function objects_update($ps_xlsx) {
    ...
    /**
     * KULTMMS-2387
     * Expected column mapping for the ODS import file.
     *
     * IMPORTANT:
     * - Column indexes are 1-based (PhpSpreadsheet convention).
     * - The import assumes that the spreadsheet strictly follows this structure.
     * - If columns are reordered, renamed or removed, the import may fail
     *   or produce inconsistent data updates.
     *
     * The mapping below documents:
     *   Index (1-based) | Excel Column Letter | Expected Header Label
     *
     * This serves as:
     *   1. Technical documentation for developers
     *   2. Validation reference for debugging
     *   3. Clear structure definition for future maintenance
     */
    mmsLog("ODS TEMPLATE STRUCTURE (MANDATORY COLUMN ORDER)", Zend_Log::INFO);
    mmsLog("The following column order MUST be respected in the ODS file.", Zend_Log::INFO);
    mmsLog("Changing column positions will break this import script.", Zend_Log::INFO);
    $va_expected_cols = [
        1  => ['A',  'ObjektID'],
        2  => ['B',  'Inventarnummer'],
        3  => ['C',  'Titel/Kurzbeschreibung'],
        4  => ['D',  'StandortID Aufbewahrungsort'],
        5  => ['E',  'StandortID Bearbeiter Aufbewahrungsort'],
        6  => ['F',  'StandortID Datum Aufbewahrungsort'],
        7  => ['G',  'StandortID Aktueller Standort'],
        8  => ['H',  'StandortID Bearbeiter Aktueller Standort'],
        9  => ['I',  'StandortID Datum Aktueller Standort'],
        10 => ['J',  'Personen ID'],
        11 => ['K',  'Rolle der Person'],
        12 => ['L',  'Zuschreibung'],
        13 => ['M',  'Ort Text'],
        14 => ['N',  'Ort Geonames'],
        15 => ['O',  'Datierung anzeige'],
        16 => ['P',  'Datum rechnerisch'],
        17 => ['Q',  'Entstehungsart'],
        18 => ['R',  'Objektart'],
        19 => ['S',  'Material/Technik'],
        20 => ['T',  'Maßfreitext'],
        21 => ['U',  'Höhe'],
        22 => ['V',  'Breite'],
        23 => ['W',  'Tiefe'],
        ...
        25 => ['Y',  'Maß geprüft'],
        26 => ['Z',  'Rahmung'],
        27 => ['AA', 'Signatur/Beschriftung'],
        28 => ['AB', 'WIO Beschreibung'],
        29 => ['AC', 'WIO Sonstige Angaben'],
        30 => ['AD', 'WIO'],
        31 => ['AE', 'WIO Art'],
        32 => ['AF', 'Provenienz'],
        33 => ['AG', 'Kulturgutschutzliste'],
        34 => ['AH', 'Erwerbungs–ID'],
        35 => ['AI', 'Leihgeber/Miteigentümer'],
        36 => ['AJ', 'Wert des Objekts original'],
        37 => ['AK', 'Wert des Objekts in EUR'],
        38 => ['AL', 'Nebenkosten in EUR'],
        39 => ['AM', 'Nebenkosten Bemerkung'],
        40 => ['AN', 'Literatur'],
        41 => ['AO', 'Inventarnummer Alt Nummer'],
        42 => ['AP', 'Inventarnummer Alt Typ'],
        43 => ['AQ', 'Bemerkung Inventarisierung'],
        44 => ['AR', 'Creditlinie'],
        45 => ['AS', 'Objetkgruppe/Konvolut ID'],
        46 => ['AT', 'Ausstellungs ID'],
        47 => ['AU', 'ToDo Inventarisierung Text'],
        48 => ['AV', 'Quelle'],
        49 => ['AW', 'Bearbeitet von'],
        50 => ['AX', 'Bearbeitet Datum'],
        51 => ['AY', 'Vokabularlisten (text)'],
        52 => ['AZ', 'ToDo Datensatz'],
        53 => ['BA', 'SAP Einzelnummer'],
        54 => ['BB', 'Sammlung ID'],
        55 => ['BC', 'UUID'],
        56 => ['BD', 'Inventarstatus des Objekts'],
        57 => ['BE', 'Datensatz geprüft'],
        58 => ['BF', 'Abzug/Auflage'],
        59 => ['BG', 'Zustand Beschreibung'],
        60 => ['BH', 'Aufbewahrung'],
        61 => ['BI', 'Negativnummer'],
    ];
    mmsLog("Objekt-Update [{$ps_xlsx}]: Erwartete Spalten (Index | Spalte | Header):", Zend_Log::INFO);
    foreach ($va_expected_cols as $idx => $def) {
        [$letter, $label] = $def;
        mmsLog(sprintf("  %02d | %s | %s", $idx, $letter, $label), Zend_Log::INFO);
    }
    $va_header_errors = [];
    foreach ($va_expected_cols as $idx => [$letter, $expectedHeader]) {
        $actual = (string)$o_sheet->getCell($letter.'1')->getValue();
        // normalize: convert newlines/tabs/NBSP to single spaces + trim
        $actual = preg_replace('/[\r\n\t]+/u', ' ', $actual);
        $actual = preg_replace('/\x{00A0}/u', ' ', $actual);   // NBSP
        $actual = preg_replace('/\s+/u', ' ', $actual);
        $actual = trim($actual);

        if ($actual !== $expectedHeader) {
            $va_header_errors[] = sprintf("%s1: expected '%s' but got '%s'", $letter, $expectedHeader, $actual);
        }
    }
    if ($va_header_errors) {
        mmsLog("Objekt-Update [{$ps_xlsx}]: Header-Check FAILED:", Zend_Log::WARN);
        foreach ($va_header_errors as $e) { mmsLog("  ".$e, Zend_Log::WARN); }
    } else {
        mmsLog("Objekt-Update [{$ps_xlsx}]: Header-Check OK", Zend_Log::INFO);
    }
    ...
    foreach ($o_sheet->getRowIterator() as $o_row) {
        ...
        $vs_literature = trim((string)$o_sheet->getCellByColumnAndRow(40, $vn_row_num));
        ...
        $vs_inv_nr_alt_nr = trim((string)$o_sheet->getCellByColumnAndRow(41, $vn_row_num));
        $vs_inv_nr_alt_type = trim((string)$o_sheet->getCellByColumnAndRow(42, $vn_row_num));
        ...
        $vs_inv_comment = trim((string)$o_sheet->getCellByColumnAndRow(43, $vn_row_num));
        ...
        $vs_creditline = trim((string)$o_sheet->getCellByColumnAndRow(44, $vn_row_num));
        ...
        $vs_todo_inv = trim((string)$o_sheet->getCellByColumnAndRow(47, $vn_row_num));
        ...
        $vs_data_source = trim((string)$o_sheet->getCellByColumnAndRow(48, $vn_row_num));
        $vs_bearbeitet_von = trim((string)$o_sheet->getCellByColumnAndRow(49, $vn_row_num));
        $vs_bearbeitet_am = mmsGetDateTimeColumnFromSheet($o_sheet, 50, $vn_row_num);
        ...
        // ToDo Datensatz (add)
        $vs_todo_datensatz = trim((string)$o_sheet->getCellByColumnAndRow(52, $vn_row_num));
        ...
        $vs_sap_einzelnr = trim((string)$o_sheet->getCellByColumnAndRow(53, $vn_row_num));
        ...
        // UUID (replace)
        $vs_uuid = trim((string)$o_sheet->getCellByColumnAndRow(55, $vn_row_num));
        if(strlen($vs_uuid)>0) {
            $t_object->replaceAttribute(array(
                'uuid' => $vs_uuid,
            ),'uuid');
        ...
        $vs_inventory_state = trim((string)$o_sheet->getCellByColumnAndRow(56, $vn_row_num));
        ...
        $vs_datensatz_geprueft = trim((string)$o_sheet->getCellByColumnAndRow(57, $vn_row_num));
        ...
        // Abzug/Auflage (add)
        $vs_abzug_auflage = trim((string)$o_sheet->getCellByColumnAndRow(58, $vn_row_num));
        ...
        $vs_zustand_beschreibung = trim((string)$o_sheet->getCellByColumnAndRow(59, $vn_row_num));
        ...
        // Aufbewahrung (add)
        $vs_aufbewahrung = trim((string)$o_sheet->getCellByColumnAndRow(60, $vn_row_num));
        ...
        // Negativnummer (add)
        $vs_negativnummer = trim((string)$o_sheet->getCellByColumnAndRow(61, $vn_row_num));
        ...
        $vs_entity_attribution = trim((string)$o_sheet->getCellByColumnAndRow(12, $vn_row_num));
        ...
        $vs_objektgruppen = trim((string)$o_sheet->getCellByColumnAndRow(45, $vn_row_num));
        ...
        // Ausstellungen (add)
        $vs_ausstellungen = trim((string)$o_sheet->getCellByColumnAndRow(46, $vn_row_num));
        ...
        // Schlagworte (add)
        $vs_schlagworte = trim((string)$o_sheet->getCellByColumnAndRow(51, $vn_row_num));
        ...
        // Sammlung (replace)
        $vs_sammlung = trim((string)$o_sheet->getCellByColumnAndRow(54, $vn_row_num));
